...
for item in sys.argv:
    li.append(item)
cmd0 = "Command: " + ' '.join(li)
os.system("echo " + cmd0)
dic_sample = {}
if args.L == "SE":
    with open(args.Pathfile,"r") as file_in:
        for line in file_in:
...
# 2.filter
os.system("echo '===============================2nd step========================='")
os.chdir(workpath)
os.system("mkdir 2.filter && cd 2.filter")
for key,value in dic_sample.items():
    os.chdir(workpath + "/2.filter")
    os.system("mkdir " + key)
    os.chdir(workpath + "/2.filter/" + key)
    if args.L == "SE":
        if args.f == "SOAPnuke":
            cmd3 = "SOAPnuke filter -l 15 -q 0.2 -n 0.05 -Q " + str(args.Q) + " -1 " + value + " > log.txt 2>&1 &"
            os.system("echo " + cmd3)
            os.system(cmd3)
        ...
        if args.f == "SOAPnuke":
            cmd3 = "SOAPnuke filter -l 15 -q 0.2 -n 0.05 -Q " + str(args.Q) + " -1 " + value[0] + " -2 " + value[1] + " > log.txt 2>&1 &"
            os.system("echo " + cmd3)
            os.system(cmd3)
        elif args.f == "TrimGalore":
            if args.Q == 1:
...
for key,value in dic_sample.items():
    ...
    if args.L == "SE":
        ...
        if args.Q == 1:
            cmd4 = "hisat2 -k 1 -t -p 12 --phred64 -x " + args.x + " -S accepted_hits.sam -q --no-unal --dta --un-conc-gz unmapped.sam.gz -1 " +\
                   "../../2.filter/" + key + "/*_1.*.gz" + " -2 ../../2.filter/" + key + "/*_2.*.gz" + " > log.txt 2>&1"
            os.system("echo " + cmd4)
            os.system(cmd4)
        elif args.Q == 2:
    ...
    os.system("samtools index accepted_hits.sorted.bam")
...
# 4.Gene_Expression
os.system("echo '===============================4th step========================='")
os.chdir(workpath)
os.system("mkdir 4.Gene_Expression && cd 4.Gene_Expression")
for key,value in dic_sample.items():
    os.chdir(workpath + "/4.Gene_Expression")
    os.system("mkdir " + key)
    os.chdir(workpath + "/4.Gene_Expression/" + key)
    ...
    os.system("echo " + cmd6)
    os.system(cmd6)
    cmd7 = "perl " + filepath + "/scripts/sumifs.pl gene_abund.tab gene_abund.uniq.tab && sort gene_abund.uniq.tab > gene_abund.uniq.sorted.tab"
    os.system(cmd7)
os.chdir(workpath + "/4.Gene_Expression/" + list(dic_sample.keys())[0])
...
os.system("perl " + filepath + "/scripts/merged.pl sample_list.txt pre_merged.gene_fpkm.txt merged.gene_fpkm.txt")
os.system("echo '4.Gene_Expression: finished!'")

# 5.StringTie_assembly
os.system("echo '===============================5th step========================='")
os.chdir(workpath)
os.system("mkdir 5.StringTie_assembly && cd 5.StringTie_assembly")
for key,value in dic_sample.items():
    os.chdir(workpath + "/5.StringTie_assembly")
    os.system("mkdir " + key)
    os.chdir(workpath + "/5.StringTie_assembly/" + key)
...
os.system("perl " + filepath + "/scripts/0b_gene_locus.1.pl merged.gtf merged.locus")
cmd11 = "gffread -w isoforms.FASTA -g " + args.s + " merged.gtf"
os.system(cmd11)
os.system("perl " + filepath + "/scripts/FASTA_to_fa.1.pl isoforms.FASTA isoforms.fa")
os.system("perl " + filepath + "/scripts/fa_to_Tab.pl isoforms.fa isoforms.Tab")
os.system("perl " + filepath + "/scripts/1_TCONS_XLOC_seq.pl merged.gtf isoforms.Tab isoforms.1.Tab")
os.system("echo '5.StringTie_assembly: finished!'")

# 6.Transcript_assembly_expression
os.system("echo '===============================6th step========================='")
os.chdir(workpath)
os.system("mkdir 6.Transcript_assembly_expression && cd 6.Transcript_assembly_expression")
for key,value in dic_sample.items():
    os.chdir(workpath + "/6.Transcript_assembly_expression")
    os.system("mkdir " + key)
    os.chdir(workpath + "/6.Transcript_assembly_expression/" + key)
    ...
    os.system("echo " + cmd12)
    os.system(cmd12)
    cmd13 = "perl " + filepath + "/scripts/sumifs.pl gene_abund.tab gene_abund.uniq.tab && sort gene_abund.uniq.tab > gene_abund.uniq.sorted.tab"
    os.system(cmd13)
os.chdir(workpath + "/6.Transcript_assembly_expression/" + list(dic_sample.keys())[0])
os.system("prepDE.py -i ../")
os.chdir(workpath + "/6.Transcript_assembly_expression/")
cmd14 = "paste "
for key in dic_sample.keys():
