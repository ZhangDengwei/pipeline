					...
					if mate_chromosome == "=" and insert_size != '0' and int(map_quality) >= 50:
						if int(leftmost_pos) > int(mate_read_leftmost_pos):
							...
							if gap <= 0 and abs(gap) > overlap_threshold:
								...
								# record each spot, the end position of read_1 and start position of read_2
								iv_read1_end = HTSeq.GenomicPosition(chromosome, end_position - overlap_threshold, "+")
								iv_read2_start = HTSeq.GenomicPosition(chromosome, int(mate_read_leftmost_pos), "+")
								ga[iv_read1_end] += 1
								ga[iv_read2_start] += 1
								...
								# record each spot, the end position of read_1 and start position of read_2
								iv_read1_end = HTSeq.GenomicPosition(chromosome, end_position, "+")
								iv_read2_start = HTSeq.GenomicPosition(chromosome, int(mate_read_leftmost_pos), "+")
								ga[iv_read1_end] += 1
								ga[iv_read2_start] += 1
								print(read_name, chromosome, end_position, mate_read_leftmost_pos, gap, sep="\t", file=fo)
...
def output_offtarget(SAMfile, search_radius, reference_genome, GTF_file, targetsite, gap_threshold, overlap_threshold, mismatch_threshold, 
	...
	for iv, value in ga.steps():
		if value:
			# To tile each spot of mapped reads
			for position in range(iv.start, iv.end):
				gene_name = [value for iv, value in ga_gene[iv].steps()][0]
				gene_name = gene_name if gene_name else '*'
				# In case bigger gap exists when searching
				window_start = position - search_radius - gap_threshold
				window_end = position + search_radius + gap_threshold
				window_sequence = get_sequence(reference_genome, iv.chrom, window_start, window_end)
				...
				if final_alignment_seq and final_alignment_strand == "+":
					absolute_start = window_start + int(final_alignment_start)
					absolute_end = window_start + int(final_alignment_end)
					...
					absolute_start = window_end - int(final_alignment_end)
					absolute_end = window_end - int(final_alignment_start)
					absolute_strand = final_alignment_strand
